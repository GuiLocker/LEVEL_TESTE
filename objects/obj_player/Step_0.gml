if (global.player_liberado = true)
{
	...
	if (!place_meeting(x,y + 10, obj_espinho))
	{
		...
		if (_jump && place_meeting(x, y + 1, obj_colisor_chao))
		{
			...
			if (!audio_is_playing(snd_jump_player) && audio_is_playing(snd_walk_player))
			{
				audio_play_sound(snd_jump_player, 10, 0, 0.5)
			}
		
		
			if (!audio_is_playing(snd_jump_player))
			{
		...
		if (_direcao_horizontal = 1 && gravidade = 0)
		{
			...
			if (!audio_is_playing(snd_walk_player))
			{
				audio_play_sound(snd_walk_player, 10, 0)
			}
